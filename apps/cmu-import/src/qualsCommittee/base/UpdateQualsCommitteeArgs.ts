/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QualsCommitteeWhereUniqueInput } from "./QualsCommitteeWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { QualsCommitteeUpdateInput } from "./QualsCommitteeUpdateInput";

@ArgsType()
class UpdateQualsCommitteeArgs {
  @ApiProperty({
    required: true,
    type: () => QualsCommitteeWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => QualsCommitteeWhereUniqueInput)
  @Field(() => QualsCommitteeWhereUniqueInput, { nullable: false })
  where!: QualsCommitteeWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => QualsCommitteeUpdateInput,
  })
  @ValidateNested()
  @Type(() => QualsCommitteeUpdateInput)
  @Field(() => QualsCommitteeUpdateInput, { nullable: false })
  data!: QualsCommitteeUpdateInput;
}

export { UpdateQualsCommitteeArgs as UpdateQualsCommitteeArgs };
