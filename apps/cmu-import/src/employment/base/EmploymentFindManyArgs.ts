/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EmploymentWhereInput } from "./EmploymentWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { EmploymentOrderByInput } from "./EmploymentOrderByInput";

@ArgsType()
class EmploymentFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => EmploymentWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => EmploymentWhereInput, { nullable: true })
  @Type(() => EmploymentWhereInput)
  where?: EmploymentWhereInput;

  @ApiProperty({
    required: false,
    type: [EmploymentOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [EmploymentOrderByInput], { nullable: true })
  @Type(() => EmploymentOrderByInput)
  orderBy?: Array<EmploymentOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { EmploymentFindManyArgs as EmploymentFindManyArgs };
