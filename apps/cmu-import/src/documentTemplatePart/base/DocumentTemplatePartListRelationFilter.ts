/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DocumentTemplatePartWhereInput } from "./DocumentTemplatePartWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class DocumentTemplatePartListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => DocumentTemplatePartWhereInput,
  })
  @ValidateNested()
  @Type(() => DocumentTemplatePartWhereInput)
  @IsOptional()
  @Field(() => DocumentTemplatePartWhereInput, {
    nullable: true,
  })
  every?: DocumentTemplatePartWhereInput;

  @ApiProperty({
    required: false,
    type: () => DocumentTemplatePartWhereInput,
  })
  @ValidateNested()
  @Type(() => DocumentTemplatePartWhereInput)
  @IsOptional()
  @Field(() => DocumentTemplatePartWhereInput, {
    nullable: true,
  })
  some?: DocumentTemplatePartWhereInput;

  @ApiProperty({
    required: false,
    type: () => DocumentTemplatePartWhereInput,
  })
  @ValidateNested()
  @Type(() => DocumentTemplatePartWhereInput)
  @IsOptional()
  @Field(() => DocumentTemplatePartWhereInput, {
    nullable: true,
  })
  none?: DocumentTemplatePartWhereInput;
}
export { DocumentTemplatePartListRelationFilter as DocumentTemplatePartListRelationFilter };
