/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EventScheduleWhereInput } from "./EventScheduleWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { EventScheduleOrderByInput } from "./EventScheduleOrderByInput";

@ArgsType()
class EventScheduleFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => EventScheduleWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => EventScheduleWhereInput, { nullable: true })
  @Type(() => EventScheduleWhereInput)
  where?: EventScheduleWhereInput;

  @ApiProperty({
    required: false,
    type: [EventScheduleOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [EventScheduleOrderByInput], { nullable: true })
  @Type(() => EventScheduleOrderByInput)
  orderBy?: Array<EventScheduleOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { EventScheduleFindManyArgs as EventScheduleFindManyArgs };
