/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { StudentPubWhereUniqueInput } from "./StudentPubWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { StudentPubUpdateInput } from "./StudentPubUpdateInput";

@ArgsType()
class UpdateStudentPubArgs {
  @ApiProperty({
    required: true,
    type: () => StudentPubWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => StudentPubWhereUniqueInput)
  @Field(() => StudentPubWhereUniqueInput, { nullable: false })
  where!: StudentPubWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => StudentPubUpdateInput,
  })
  @ValidateNested()
  @Type(() => StudentPubUpdateInput)
  @Field(() => StudentPubUpdateInput, { nullable: false })
  data!: StudentPubUpdateInput;
}

export { UpdateStudentPubArgs as UpdateStudentPubArgs };
